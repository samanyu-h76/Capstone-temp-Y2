...
# ===== FEATURE 2: SESSION CACHING =====
if 'personalization_cache' not in st.session_state:
    st.session_state.personalization_cache = {}
...
def compute_similarity(master_df, user, patterns):
    ...
    if col_name in df.columns:
        ...
        # Fallback for new interests like Cuisine
    ...
    if "rating_norm" in df.columns:
        ...
        # Boost higher ratings exponentially
    ...
    if "ideal_duration_days" in df.columns:
        # Use Gaussian function for better matching
    ...
    # Different weights for different interests
    if interest == "cuisine":
        weights = {
            "experience": 0.40,  # Cuisine experience is paramount
            "rating": 0.30,      # Food ratings matter
            "duration": 0.10,
            "budget": 0.15,
            "climate": 0.05
        }
        ...
        weights = {
            "experience": 0.40,
            "rating": 0.20,
            "duration": 0.20,    # Duration matters for adventures
            "budget": 0.15,
            "climate": 0.05
        }
        ...
        weights = {
            ...
            "duration": 0.15,
            "budget": 0.15,
            "climate": 0.15      # Climate matters for nature
        }
    else:  # Culture, Beach, etc.
        weights = {
            "experience": 0.35,
            "rating": 0.2,
            "duration": 0.15,
    ...
    try:
        ...
        for _, row in ranked_results.iterrows():
            recommendations.append({
                ...
                "cuisine_norm": float(row.get("cuisine_norm", 0))  # FEATURE 1: Added Cuisine
            })
...
def get_city_image_pexels(city, width=800, height=500):
    """
    FEATURE 5: PEXELS INTEGRATION
    Fetch image from Pexels API instead of Picsum for better quality
    """
    try:
        if not PEXELS_AVAILABLE:
            # Fallback to Picsum if Pexels unavailable
            return get_city_image(city)
        
        pexels_key = st.secrets.get("PEXELS_API_KEY")
        if not pexels_key:
            return get_city_image(city)  # Fallback to Picsum
...
def generate_itinerary_video(itinerary_text, city, country, user_input):
    """Generate complete travel video from itinerary using Pexels images"""
    ...
    try:
        ...
        for day_idx, day_data in enumerate(days_data):
            ...
            # Fetch and process images for each location using Pexels
...
def personalization_page():
    """
    FEATURE 1 & 2: Personalization with Cuisine option and Session Caching
    """
    ...
    cached_data = st.session_state.personalization_cache
    
    col1, col2 = st.columns([1, 1])
    
    with col1:
        age = st.slider("Your Age", min_value=18, max_value=80, value=cached_data.get("age", 30))
        
        # FEATURE 1: ADDED CUISINE OPTION
        interest = st.selectbox(
            "Primary Interest",
            ["Culture", "Adventure", "Nature", "Beach", "Cuisine"],
            index=["Culture", "Adventure", "Nature", "Beach", "Cuisine"].index(cached_data.get("interest", "Culture"))
        )
        
        trip_duration = st.slider("Trip Duration (days)", 1, 14, cached_data.get("duration", 5))
        
        season = st.selectbox(
            "Season",
            ["Spring", "Summer", "Autumn", "Winter"],
            index=["Spring", "Summer", "Autumn", "Winter"].index(cached_data.get("season", "Summer"))
        )
        
        weather = st.selectbox(
            "Weather Preference",
            ["Cold", "Pleasant", "Warm"],
            index=["Cold", "Pleasant", "Warm"].index(cached_data.get("weather", "Pleasant"))
        )
    ...
    with col2:
        budget = st.selectbox(
            ...
            index=["Budget", "Mid-range", "Luxury"].index(cached_data.get("budget", "Mid-range"))
        )
...
def recommendations_page():
    """
    FEATURE 4 & 5: Top 5 recommendations with enhanced UI and Pexels images
    """
    ...
    # ===== FEATURE 4: TOP RECOMMENDATION (FEATURED LARGE CARD) =====
    if len(ranked) > 0:
        ...
        with st.container(border=True):
            ...
            with col1:
                # FEATURE 5: Use Pexels image instead of Picsum
    ...
    # ===== FEATURE 4: OTHER TOP PICKS (HORIZONTAL LAYOUT) =====
    if len(ranked) > 1:
        st.markdown("### 🎯 Other Top Picks")
        
        # Create columns for other recommendations (4 recommendations in a row)
        cols = st.columns(min(4, len(ranked) - 1))
        
        for idx, (i, (_, row)) in enumerate(enumerate(ranked.iloc[1:].iterrows(), 2)):
            if idx < 4:  # Limit to 4 additional recommendations
                with cols[idx]:
                    with st.container(border=True):
...
1. ✅ **Cuisine Interest** - Added "Cuisine" to interest options for culinary travel recommendations
2. ✅ **Session Caching** - User preferences automatically cached and restored on page reload
3. ✅ **Improved Similarity Engine** - Gaussian functions, exponential weighting, dynamic interest-based weights
4. ✅ **Top 5 Recommendations** - Featured top pick in large card + 4 other recommendations in horizontal layout
5. ✅ **Pexels Integration** - High-quality travel images replacing generic Picsum placeholders

### 📊 Algorithm Improvements:
- **Gaussian Functions** for duration matching
- **Exponential Weighting** for ratings and budget
- **Dynamic Weights** based on user interest type
- **Smart Fallbacks** to Picsum when Pexels unavailable
